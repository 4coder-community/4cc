...
enum BuildMode {
    MODE_DEBUG = DEBUG_INFO | INTERNAL,
    MODE_RELEASE = OPTIMIZATION | SHIP,
    MODE_RELEASE_DEBINFO = MODE_DEBUG | MODE_RELEASE,
};
...
dispatch_build(Arena* arena, BuildFlags flags) {
    ...
    // Build 4ed_app
    {
        ...
        flags.opts = flags.type | (flags.opts & VERBOSE) | OPTS | SHARED_CODE;

        build(arena, "4ed_app" DLL, files, defines, exports, includes, flags);
    ...
    // Build Platform Layer
    {
        ...
        flags.opts = flags.type | (flags.opts & VERBOSE) | OPTS | LIBS | ICON;

        build(arena, "4ed", files, defines, {}, includes, flags);
...
BuildFlags parse_flags(int argc, const char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            flags.opts |= VERBOSE;
        } else if (string_has_prefix(arg, "-m=") || string_has_prefix(arg, "--mode=")) {
            String_Const_char param = get_flag_value(arg);
            if (param == "release") {
                flags.type = MODE_RELEASE;
            } else if (param == "debug") {
                flags.type = MODE_DEBUG;
            } else if (param == "release_debinfo") {
                flags.type = MODE_RELEASE_DEBINFO;
            } else {
                fprintf(stderr, "build: invalid mode '%.*s'\n", string_expand(param));
                usage(stderr);
                exit(1);
            }
        } else if (string_has_prefix(arg, "--gl-api=")) {
            String_Const_char param = get_flag_value(arg);
            if (param == "DX11" && OS_WINDOWS) {
...
bool set_common_dirs(Arena* arena) {
    ...
    // Get parent folder of cwd
    for (int i = cwd_size - 1; i >= 0; --i) {
        if (cwd[i] == (OS_WINDOWS ? '\\' : '/')) {
            cwd[i] = '\0';
            break;
        }
